...
class SyncUpstreamsTask(TaskBase):
    ...
    def execute(self) -> int:
        ...
        for source in self.sources:
            try:
                ...
                raise TaskException(
                    f"Failed to fetch upstream content from {source}: {e}"
                )
        return const.SUCCESS_EXIT_CODE

    # QUESTION(jpower432): What should the be behavior if the same model
    # or model with the same name imported by multiple sources?
    def _fetch(self, source: str) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=self.working_dir) as temporary_git_dir:
            if "@" not in source:
                raise TaskException(
                    f"Invalid source {source}. Source must be of the form <repo_url>@<ref>"
                )
            repo_url, ref = source.split("@")

            upstream_trestle_workspace: pathlib.Path = pathlib.Path(temporary_git_dir)
